...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if(co.sd<100.0){
        ...
        //shadows
        vec3 newRayOrigin = p + normal * 0.001;
        float shadowRayLength= rayMarch(newRayOrigin, ld, 0.0, 100.0).sd;
        if(shadowRayLength < length(lo - newRayOrigin))
            diffuse *= ambient;
        
        col = co.sc * (diffuse + ambient);
    }
